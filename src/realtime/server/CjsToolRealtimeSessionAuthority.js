...
export class CjsToolRealtimeSessionAuthority
{
    ...
    AddGrant(grant)
    {
        if (!CjsToolRealtimeProtocol.isRecord(grant))
        {
            throw new TypeError("Realtime capability grant must be an object");
        }

        CjsToolRealtimeProtocol.assertString(grant.capability, "capability", 32, 2048);

        if (!CjsToolRealtimeProtocol.isRecord(grant.actor))
        {
            throw new TypeError("Realtime capability actor must be an object");
        }

        CjsToolRealtimeProtocol.assertServiceId(grant.actor.id);
        CjsToolRealtimeProtocol.assertName(grant.actor.kind, "actor kind");
        ...
        const scopes = CjsToolRealtimeSessionAuthority.normalizeScopes(grant.scopes ?? {});
        ...
        this.#grants.set(CjsToolRealtimeSessionAuthority.digestCapability(grant.capability), record);
    ...
    {
        return this.#grants.delete(CjsToolRealtimeSessionAuthority.digestCapability(capability));
    }
    ...
    Authenticate(capability, { origin = null } = {})
    {
        ...
        const grantId = CjsToolRealtimeSessionAuthority.digestCapability(capability);
        ...
        else
        {
            ...
            try
            {
                normalizedOrigin = CjsToolRealtimeSessionAuthority.normalizeOrigin(origin);
            }
            catch
            {
                throw CjsToolRealtimeSessionAuthority.unauthorized();
    ...
    static normalizeScopes(value)
    {
        ...
        if (!CjsToolRealtimeProtocol.isRecord(serviceValues))
        {
        ...
        for (const [ serviceId, source ] of Object.entries(serviceValues))
        {
            ...
            if (!CjsToolRealtimeProtocol.isRecord(source))
            {
                throw new TypeError("Realtime service scope must be an object");
            }

            const topics = CjsToolRealtimeSessionAuthority.normalizeNameArray(
                source.topics ?? [],
                "topic",
            );
            const commands = CjsToolRealtimeSessionAuthority.normalizeNameArray(
                source.commands ?? [],
                "command",
            );
    ...
    static normalizeNameArray(value, label)
    {
        ...
        {
            CjsToolRealtimeProtocol.assertName(name, label);
    ...
    static unauthorized()
    {
        return new CjsToolRealtimeError("unauthorized", "Realtime capability is not authorized", {
