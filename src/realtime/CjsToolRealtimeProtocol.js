...
export class CjsToolRealtimeProtocol
{
    ...
    } = {})
    {
        ...
        {
            throw new CjsToolRealtimeError("invalid_message", "Realtime messages must be text", {
        ...
        {
            throw new CjsToolRealtimeError("message_too_large", "Realtime message exceeds the byte limit", {
                connectionUsable: false,
                closeCode: 1009,
            });
        }
        ...
        {
            throw new CjsToolRealtimeError("invalid_json", "Realtime message is not valid JSON", {
                connectionUsable: false,
                closeCode: 1002,
                cause: error,
            });
        ...
        CjsToolRealtimeProtocol.validateJson(value, { maxDepth, maxNodes });

        if (!CjsToolRealtimeProtocol.isRecord(value))
        {
            throw new CjsToolRealtimeError("invalid_message", "Realtime message must be an object", {
                connectionUsable: false,
                closeCode: 1002,
            });
        }
    ...
    {
        if (!CjsToolRealtimeProtocol.isRecord(value) || typeof value.type !== "string")
        {
            throw new CjsToolRealtimeError("invalid_message", "Realtime message type is required", {
                connectionUsable: false,
                closeCode: 1002,
            });
        }
        ...
        {
            throw new CjsToolRealtimeError("hello_required", "The first realtime message must be hello", {
                connectionUsable: false,
                closeCode: 1002,
            });
        }
        ...
        if (authenticated && value.type === "hello")
        {
            throw new CjsToolRealtimeError("unexpected_hello", "Realtime hello was already accepted", {
                connectionUsable: false,
                closeCode: 1002,
            });
        }
        ...
        if (value.type === "hello")
        {
            ...
            {
                throw new CjsToolRealtimeError("unsupported_version", "Unsupported realtime protocol version", {
                    connectionUsable: false,
                    closeCode: 1002,
                });
            }

            CjsToolRealtimeProtocol.assertString(value.capability, "capability", 1, 2048);

            if (value.client !== undefined && !CjsToolRealtimeProtocol.isRecord(value.client))
            {
                throw new CjsToolRealtimeError("invalid_request", "hello.client must be an object");
            }
            ...
            return Object.freeze({
                ...
                client: value.client === undefined
                    ? null
                    : CjsToolRealtimeProtocol.cloneJson(value.client),
            });
        }

        CjsToolRealtimeProtocol.assertRequestId(value.requestId);

        if ([ "subscribe", "subscribe-targeted" ].includes(value.type))
        {
            CjsToolRealtimeProtocol.assertServiceId(value.serviceId);

            if (!Array.isArray(value.topics) || value.topics.length === 0)
            {
                throw new CjsToolRealtimeError("invalid_request", "subscribe.topics must be a non-empty array");
            }

            const topics = value.topics.map(topic =>
            {
                CjsToolRealtimeProtocol.assertName(topic, "topic");
            ...
            {
                throw new CjsToolRealtimeError("invalid_request", "subscribe.topics must be unique");
            }

            const targeted = value.type === "subscribe-targeted";

            if (targeted && !CjsToolRealtimeProtocol.isRecord(value.target))
            {
            ...
            if (targeted)
            {
                message.target = Object.freeze(CjsToolRealtimeProtocol.cloneJson(value.target));
            }
        ...
        {
            CjsToolRealtimeProtocol.assertString(value.subscriptionId, "subscriptionId", 1, 128);
        ...
        {
            CjsToolRealtimeProtocol.assertServiceId(value.serviceId);
            CjsToolRealtimeProtocol.assertName(value.action, "action");

            if (value.operationId !== undefined && value.operationId !== null)
            {
                CjsToolRealtimeProtocol.assertString(value.operationId, "operationId", 1, 128);
            }

            CjsToolRealtimeProtocol.validateJson(value.data ?? null);
            ...
            return Object.freeze({
                ...
                data: CjsToolRealtimeProtocol.cloneJson(value.data ?? null),
            });
        }

        throw new CjsToolRealtimeError("invalid_message", "Unsupported realtime message type", {
            connectionUsable: false,
            closeCode: 1002,
        });
    }
    ...
    static normalizeServiceDescription(value)
    {
        ...
        CjsToolRealtimeProtocol.assertServiceId(value.id);
        CjsToolRealtimeProtocol.assertName(value.family, "family");
        CjsToolRealtimeProtocol.assertName(value.kind, "kind");
        ...
        const topics = value.topics.map(entry =>
        {
            ...
            if (!CjsToolRealtimeProtocol.isRecord(topic))
            {
                throw new TypeError("Realtime service topic must be a string or object");
            }

            CjsToolRealtimeProtocol.assertName(topic.name, "topic name");
        ...
        const commands = (value.commands ?? []).map(entry =>
        {
            ...
            if (!CjsToolRealtimeProtocol.isRecord(command))
            {
                throw new TypeError("Realtime service command must be a string or object");
            }

            CjsToolRealtimeProtocol.assertName(command.name, "command name");
        ...
        if (!CjsToolRealtimeProtocol.isRecord(subscriptionValue))
        {
        ...
        {
            CjsToolRealtimeProtocol.assertName(subscriptionTarget, "subscription target");
        }
    ...
    {
        CjsToolRealtimeProtocol.validateJson(value);
    ...
    static canonicalStringify(value)
    {
        ...
        return JSON.stringify(CjsToolRealtimeProtocol.#canonicalize(value));
    }
    ...
    static validateJson(value, { maxDepth = 32, maxNodes = 16384 } = {})
    {
        ...
        CjsToolRealtimeProtocol.#validateJsonValue(value, 0, maxDepth, maxNodes, state);
    ...
    static assertServiceId(value)
    {
        ...
        {
            throw new CjsToolRealtimeError("invalid_request", "Invalid realtime service ID");
        }
    ...
    static assertName(value, label)
    {
        ...
        {
            throw new CjsToolRealtimeError("invalid_request", `Invalid realtime ${label}`);
        }
    ...
    static assertRequestId(value)
    {
        ...
        {
            throw new CjsToolRealtimeError("invalid_request", "Invalid realtime requestId");
        }
    ...
    static assertString(value, label, minimum, maximum)
    {
        ...
        {
            throw new CjsToolRealtimeError("invalid_request", `${label} must be a bounded string`);
        }
    ...
    static #canonicalize(value)
    {
        ...
        {
            return value.map(entry => CjsToolRealtimeProtocol.#canonicalize(entry));
        }
        ...
        if (CjsToolRealtimeProtocol.isRecord(value))
        {
            ...
            {
                result[key] = CjsToolRealtimeProtocol.#canonicalize(value[key]);
            }
    ...
    static #validateJsonValue(value, depth, maxDepth, maxNodes, state)
    {
        ...
        {
            throw new CjsToolRealtimeError("invalid_request", "JSON value exceeds structural limits");
        }
        ...
        if (typeof value === "number")
        {
            ...
            {
                throw new CjsToolRealtimeError("invalid_request", "JSON numbers must be finite");
            }
        ...
        {
            throw new CjsToolRealtimeError("invalid_request", "Value is not JSON-compatible");
        }

        if (!Array.isArray(value) && !CjsToolRealtimeProtocol.isRecord(value))
        {
            throw new CjsToolRealtimeError("invalid_request", "JSON objects must be plain records");
        }

        if (state.seen.has(value))
        {
            throw new CjsToolRealtimeError("invalid_request", "JSON value must not contain a cycle");
        }
        ...
        {
            CjsToolRealtimeProtocol.#validateJsonValue(
                entry,
                depth + 1,
                maxDepth,
                maxNodes,
