...
import { CjsToolRealtimeServiceContext } from "../server/CjsToolRealtimeServiceContext.js";
import { CjsToolRealtimeSerialLane } from "./CjsToolRealtimeSerialLane.js";

/** Owns lifecycle, publication order, cursors, and subscribers for one service. */
export class CjsToolRealtimeServiceController
{
    ...
    constructor({ service, description, clock, createId, maxPending })
    {
        ...
        this.#lane = new CjsToolRealtimeSerialLane({ maxPending });
    ...
    async Subscribe(connection, topics, target, requestId)
    {
        ...
        return this.#lane.Enqueue(async () =>
        {
            ...
            for (const topic of topics)
            {
                ...
                {
                    throw new CjsToolRealtimeError("topic_not_found", "Realtime topic was not found");
                }
            }
            ...
            const request = Object.freeze({
                ...
                target: target === null
                    ? null
                    : Object.freeze(CjsToolRealtimeProtocol.cloneJson(target)),
                actor: Object.freeze(CjsToolRealtimeProtocol.cloneJson(connection.session.actor)),
            });
            ...
            if (target !== null && !CjsToolRealtimeProtocol.isRecord(canonicalTarget))
            {
                throw new CjsToolRealtimeError(
            ...
            const record = {
                ...
                target: canonicalTarget === null
                    ? null
                    : Object.freeze(CjsToolRealtimeProtocol.cloneJson(canonicalTarget)),
                sequence: 0,
            ...
            const data = {
                subscriptionId,
                service: CjsToolRealtimeProtocol.serviceIdentity(this.description),
    ...
    async ExecuteCommand({ actor, action, operationId, data })
    {
        ...
        return this.#lane.Enqueue(async () =>
        {
            ...
            if (!declaration)
            {
                throw new CjsToolRealtimeError("action_not_found", "Realtime action was not found");
            }

            if (declaration.operationRequired && !operationId)
            {
                throw new CjsToolRealtimeError("operation_required", "Realtime operationId is required");
            }
            ...
            const value = await this.#RunInline(generation, actor, context =>
                this.#service.HandleCommand(Object.freeze({
                    ...
                    action,
                    operationId,
                    data: CjsToolRealtimeProtocol.cloneJson(data),
                    actor: Object.freeze(CjsToolRealtimeProtocol.cloneJson(actor)),
                }), context));

            return CjsToolRealtimeServiceController.normalizeCommandResult(value);
        });
    }
    ...
    async GetSnapshot(request)
    {
        ...
        return this.#lane.Enqueue(async () =>
        {
            ...
            const value = await this.#service.GetSnapshot(Object.freeze({
                actor: Object.freeze(CjsToolRealtimeProtocol.cloneJson(request.actor)),
            }));
            ...
            return Object.freeze({
                ...
                payload: Object.freeze({
                    ...
                    data: CjsToolRealtimeProtocol.cloneJson(value),
                }),
            });
        });
    }
    ...
    #CreateExternalContext(actor)
    {
        ...
        return new CjsToolRealtimeServiceContext({
            actor,
    ...
    async #RunInline(generation, actor, callback)
    {
        ...
        context = new CjsToolRealtimeServiceContext({
            actor,
            signal: this.#abortController?.signal ?? AbortSignal.abort(),
            clock: this.#clock,
            createId: this.#createId,
    ...
    #PublishInline(topic, data, options, actor)
    {
        ...
        CjsToolRealtimeProtocol.validateJson(data);
        const schema = options.schema ?? `${this.description.family}.event`;
        const version = options.version ?? this.description.familyVersion;

        CjsToolRealtimeProtocol.assertName(schema, "payload schema");

        if (!Number.isSafeInteger(version) || version < 1)
        {
            throw new CjsToolRealtimeError("invalid_request", "Event payload version is invalid");
        }
