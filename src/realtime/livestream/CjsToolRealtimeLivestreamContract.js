...
export class CjsToolRealtimeLivestreamContract
{
    ...
    static normalizeActivity(topic, value)
    {
        ...
        const result = {
            ...CjsToolRealtimeLivestreamContract.normalizeEventBase(value),
            actor: CjsToolRealtimeLivestreamContract.normalizeActor(value.actor, {
                nullable: [
        ...
        {
            result.subscription = CjsToolRealtimeLivestreamContract.normalizeSubscription(
                value.subscription,
            );
        }
        else if (topic === LIVESTREAM_ACTIVITY_TOPICS.SUBSCRIPTION_GIFTED)
        {
            result.gift = CjsToolRealtimeLivestreamContract.normalizeGift(value.gift);
        }
        else if (topic === LIVESTREAM_ACTIVITY_TOPICS.RAID_RECEIVED)
        {
            result.raid = CjsToolRealtimeLivestreamContract.normalizeRaid(value.raid);
        }
        else if (topic === LIVESTREAM_ACTIVITY_TOPICS.CONTRIBUTION_RECEIVED)
        {
            result.contribution = CjsToolRealtimeLivestreamContract.normalizeContribution(
                value.contribution,
            );
        }
        else if (topic === LIVESTREAM_ACTIVITY_TOPICS.REWARD_REDEEMED)
        {
            result.reward = CjsToolRealtimeLivestreamContract.normalizeReward(value.reward);
        }

        return CjsToolRealtimeLivestreamContract.freeze(result);
    }

    /** Normalizes one atomic state patch. Omitted fields remain unchanged. */
    static normalizeStateChange(value)
    {
        const base = CjsToolRealtimeLivestreamContract.normalizeEventBase(value);
        const changes = CjsToolRealtimeLivestreamContract.normalizeStream(
            value.changes,
            { partial: true },
        );
        ...
        return CjsToolRealtimeLivestreamContract.freeze({
            ...base,
            changes,
        });
    }
    ...
    {
        if (!CjsToolRealtimeProtocol.isRecord(value) || !Array.isArray(value.states))
        {
            throw new TypeError("Livestream state snapshot must contain states");
        }

        const observedAt = CjsToolRealtimeLivestreamContract.normalizeTime(
            value.observedAt,
            "observedAt",
        );
        ...
        {
            if (!CjsToolRealtimeProtocol.isRecord(entry))
            {
                throw new TypeError("Livestream snapshot state must be an object");
            }

            const source = CjsToolRealtimeLivestreamContract.normalizeSource(entry.source);
            ...
            return {
                source,
                stream: CjsToolRealtimeLivestreamContract.normalizeStream(entry.stream),
                extensions: CjsToolRealtimeLivestreamContract.normalizeExtensions(
                    entry.extensions,
                    source.provider,
                ),
            };
        ...
        return CjsToolRealtimeLivestreamContract.freeze({ observedAt, states });
    }
    ...
    static normalizeEventBase(value)
    {
        ...
        const source = CjsToolRealtimeLivestreamContract.normalizeSource(value.source);

        return {
            id: CjsToolRealtimeLivestreamContract.normalizeString(value.id, "id", 256),
            occurredAt: CjsToolRealtimeLivestreamContract.normalizeTime(
                value.occurredAt,
                "occurredAt",
            ),
            deliveryMode: CjsToolRealtimeLivestreamContract.requireValue(
                value.deliveryMode,
                [ "live" ],
                "deliveryMode",
            ),
    ...
    static normalizeSource(value)
    {
        ...
        return {
            ...
            channelId: CjsToolRealtimeLivestreamContract.normalizeString(
                value.channelId,
                "source.channelId",
                256,
            ),
            channelLogin: CjsToolRealtimeLivestreamContract.normalizeNullableString(
                value.channelLogin,
                "source.channelLogin",
                256,
            ),
            channelDisplayName: CjsToolRealtimeLivestreamContract.normalizeNullableString(
    ...
    static normalizeActor(value, { nullable = false } = {})
    {
        ...
        return {
            id: CjsToolRealtimeLivestreamContract.normalizeString(
                value.id,
                "actor.id",
                256,
            ),
            login: CjsToolRealtimeLivestreamContract.normalizeNullableString(
                value.login,
                "actor.login",
                256,
            ),
            displayName: CjsToolRealtimeLivestreamContract.normalizeNullableString(
                value.displayName,
                "actor.displayName",
                256,
            ),
    ...
    static normalizeSubscription(value)
    {
        ...
        return {
            kind: CjsToolRealtimeLivestreamContract.requireValue(
                value.kind,
                [ "gift", "new", "renewal" ],
                "subscription.kind",
            ),
            giftedBy: CjsToolRealtimeLivestreamContract.normalizeActor(value.giftedBy, {
                nullable: true,
            }),
        };
    }
    ...
    static normalizeGift(value)
    {
        ...
        return {
            count: CjsToolRealtimeLivestreamContract.normalizeInteger(
                value.count,
                "gift.count",
                { minimum: 1 },
            ),
    ...
    static normalizeRaid(value)
    {
        ...
        return {
            viewers: CjsToolRealtimeLivestreamContract.normalizeInteger(
                value.viewers,
                "raid.viewers",
                { minimum: 0 },
            ),
    ...
    static normalizeContribution(value)
    {
        ...
        return {
            amount: CjsToolRealtimeLivestreamContract.normalizeInteger(
                value.amount,
                "contribution.amount",
                { minimum: 1 },
            ),
            unit: CjsToolRealtimeLivestreamContract.normalizeString(
                value.unit,
                "contribution.unit",
                64,
            ),
            message: CjsToolRealtimeLivestreamContract.normalizeNullableString(
                value.message,
    ...
    static normalizeReward(value)
    {
        ...
        return {
            ...
            title: CjsToolRealtimeLivestreamContract.normalizeString(
                value.title,
                "reward.title",
                512,
            ),
            cost: CjsToolRealtimeLivestreamContract.normalizeInteger(
                value.cost,
                "reward.cost",
                { minimum: 0 },
            ),
            input: CjsToolRealtimeLivestreamContract.normalizeNullableString(
                value.input,
                "reward.input",
                2048,
            ),
            status: CjsToolRealtimeLivestreamContract.requireValue(
                value.status,
    ...
    static normalizeStream(value, { partial = false } = {})
    {
        ...
        for (const field of CHANGE_FIELDS)
        {
            ...
            {
                result[field] = CjsToolRealtimeLivestreamContract.normalizeStreamField(
                    field,
                    value[field],
                );
            }
    ...
    static normalizeStreamField(field, value)
    {
        ...
        {
            return CjsToolRealtimeLivestreamContract.normalizeNullableString(
                value,
                `stream.${field}`,
        ...
        if ([ "startedAt", "endedAt" ].includes(field))
        {
            return value === null
                ? null
                : CjsToolRealtimeLivestreamContract.normalizeTime(value, `stream.${field}`);
        }
        ...
        if (field === "viewers")
        {
            return value === null
                ? null
                : CjsToolRealtimeLivestreamContract.normalizeInteger(
                    value,
                    "stream.viewers",
                    { minimum: 0 },
                );
        ...
        if (field === "category")
        {
            ...
            return {
                ...
                name: CjsToolRealtimeLivestreamContract.normalizeString(
                    value.name,
                    "category.name",
                    512,
                ),
    ...
    static normalizeNullableString(value, label, maximum)
    {
        return value === null
            ? null
            : CjsToolRealtimeLivestreamContract.normalizeString(value, label, maximum);
    }
    ...
    static freeze(value)
    {
        if (value && typeof value === "object" && !Object.isFrozen(value))
        {
            ...
            {
                CjsToolRealtimeLivestreamContract.freeze(entry);
            }
        }
