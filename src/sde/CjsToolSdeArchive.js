...
export class CjsToolSdeArchive
{
    ...
    constructor(options = {})
    {
        ...
        {
            throw new TypeError("CjsToolSdeArchive requires a fetch implementation");
        }

        if (typeof this.#now !== "function")
        {
            throw new TypeError("CjsToolSdeArchive now must be a function");
        }
    ...
    async ResolveLatest()
    {
        ...
        {
            throw new TypeError("CjsToolSdeArchive now returned an invalid time");
        }
    ...
    async WriteDatabase(input, options = {})
    {
        ...
        try
        {
            database = await CjsToolSdeDatabase.open(databasePath, { readOnly: false });
        }
        catch
        {
    ...
    /** Prepares CjsToolSde input from caller-supplied ZIP bytes or a readable body. */
...
async function WriteNewDatabase(input, databasePath, importOptions, replaceInvalid)
{
    ...
    const database = await CjsToolSdeDatabase.create(temporaryPath);
    ...
    try
    {
        ...
        return CjsToolSdeDatabase.open(databasePath, { readOnly: false });
    }
    catch (error)
    {
        await database.Close();
