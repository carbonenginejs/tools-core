 ...
 *
 * That is why this is invoked from `CjsSdeDatabase` after the import commits,
...
import { BuildDnaIndex } from "./CjsSdeDnaIndex.js";
...
const DERIVATIONS = Object.freeze([
    ...
    // Takes the raw tables rather than the `CjsSde` join layer, because that
 ...
 * the cheap one.
 *
 * @param {Object} database - the CjsSdeDatabase that just committed an import
...
export async function RunDerivations(database, options = {})
{
    ...
    for (const derivation of DERIVATIONS)
    {
        ...
        try
        {
            ...
            const document = {
                ...derivation.Build(new CjsSde(tables), tables),
