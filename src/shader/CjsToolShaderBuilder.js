...
export class CjsToolShaderBuilder
{
    ...
    /**
     * This class's name, for the artifacts it writes.
     *
     * DECLARED, not read off the constructor. `this.constructor.name` was
     * what wrote the builder into every build report and every overlay's
     * provenance, and a class name is not a durable identity: a minifier
     * renames it, so a bundled build would have stamped `t` or `Bn` into
     * files that outlive the process and are compared by hash. Nothing
     * would have failed - the reports would simply have recorded a builder
     * nobody can identify.
     *
     * This is the same problem `CjsSchema` solves for models, by the same
     * means: a name stamped on the constructor rather than inferred from it,
     * so the identity survives a boundary that does not preserve function
     * names. A model registered there gets it for free and serializes as
     * `_type`; a plain class like this one declares it here, and writes the
     * same field.
     *
     * A subclass must declare its own.
     */
    static className = "CjsToolShaderBuilder";
