...
export class CjsToolWebhookStreamService
{
    ...
    } = {})
    {
        ...
        this.#description = CjsToolRealtimeProtocol.normalizeServiceDescription({
            id,
            family,
            familyVersion,
            kind,
        ...
        this.#webhookDescription = CjsToolWebhookStreamService.normalizeWebhookDescription({
            id: webhookId,
            methods,
        });
    ...
    async #Process(request)
    {
        ...
        const delivery = CjsToolWebhookStreamService.normalizeDelivery(
            value,
            this.#topicNames,
        );
            ...
            return await this.#context.Commit(async context =>
            {
                if (delivery.deliveryId !== null)
                {
                    ...
                    if (previous)
                    {
                        ...
                        return CjsToolWebhookStreamService.cloneResponse(previous.response);
                    }
                }
                ...
                return CjsToolWebhookStreamService.cloneResponse(delivery.response);
            });
        }
        catch (error)
        {
            throw CjsToolWebhookStreamService.mapCommitError(error);
        }
    }
    ...
    static normalizeDelivery(value, topicNames)
    {
        ...
        const events = sourceEvents.map(event =>
            CjsToolWebhookStreamService.normalizeEvent(event, topicNames));
        const response = CjsToolWebhookStreamService.normalizeResponse(result.response);
        const fingerprint = CjsToolRealtimeProtocol.canonicalStringify({
    ...
    {
        if (!CjsToolRealtimeProtocol.isRecord(value) || !topicNames.has(value.topic)
        ...
        {
            CjsToolRealtimeProtocol.assertName(value.schema, "payload schema");
        ...
        return Object.freeze({
            topic: value.topic,
            data: Object.freeze(CjsToolRealtimeProtocol.cloneJson(value.data)),
    ...
    static normalizeResponse(value = undefined)
    {
        ...
        if (!CjsToolRealtimeProtocol.isRecord(response))
        {
            throw new CjsToolWebhookError(
                "invalid_delivery",
        ...
        {
            body = Object.freeze(CjsToolRealtimeProtocol.cloneJson(body));
        }
    ...
    {
        if (error instanceof CjsToolWebhookError)
        {
            return error;
        }
