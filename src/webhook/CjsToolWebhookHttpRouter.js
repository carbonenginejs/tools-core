...
export class CjsToolWebhookHttpRouter
{
    ...
    } = {})
    {
        ...
        for (const endpoint of endpoints)
        {
            ...
            const description = CjsToolWebhookHttpRouter.normalizeEndpointDescription(
    ...
    CanHandle(request)
    {
        const pathname = CjsToolWebhookHttpRouter.rawPathname(request.url);
    ...
    async Handle(request, response)
    {
        ...
        {
            CjsToolWebhookHttpRouter.writeJson(response, 429, {
                error: {
        ...
        try
        {
            if (this.loopbackOnly && !CjsToolWebhookHttpRouter.isLoopback(
                request.socket?.remoteAddress,
            ))
            {
                throw new CjsToolWebhookError("forbidden", "Loopback connections only", {
                    statusCode: 403,
                });
            }

            const match = CjsToolWebhookHttpRouter.matchEndpoint(
                request.url,
                this.#route,
            );
            ...
            {
                CjsToolWebhookHttpRouter.writeJson(response, 405, {
                    error: {
            ...
            const body = await CjsToolWebhookHttpRouter.readBody(request, this.#maxBodyBytes);
            const result = await record.endpoint.HandleWebhook(Object.freeze({
                ...
                search: match.search,
                headers: CjsToolWebhookHttpRouter.normalizeHeaders(request.headers),
                body,
                receivedAt: new Date(this.#clock()).toISOString(),
                remoteAddress: request.socket?.remoteAddress ?? null,
            }));

            CjsToolWebhookHttpRouter.writeResponse(response, result);
        }
        catch (failure)
        {
            const error = CjsToolWebhookError.from(failure);

            CjsToolWebhookHttpRouter.writeJson(response, error.statusCode, {
                error: {
                    code: error.code,
    ...
    {
        const rawPathname = CjsToolWebhookHttpRouter.rawPathname(requestTarget);
        const prefix = `${route}/`;
        ...
        {
            throw new CjsToolWebhookError("invalid_path", "Webhook endpoint path is invalid");
        }
    ...
    {
        const result = CjsToolWebhookHttpRouter.normalizeResponse(value);

        if (result.body === null)
        {
            CjsToolWebhookHttpRouter.writeEmpty(response, result.statusCode, {
                "cache-control": "no-store",
            });
    ...
    static normalizeResponse(value)
    {
        ...
        if (!CjsToolRealtimeProtocol.isRecord(result)
        ...
        {
            CjsToolRealtimeProtocol.validateJson(body);
        }
