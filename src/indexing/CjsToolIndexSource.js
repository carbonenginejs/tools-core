...
export class CjsToolIndexSource
{
    ...
    async FetchCachedResolution(resolution)
    {
        ...
        // Generated by our own tools, which recorded this payload's hash
        // when they wrote it; there is nothing independent to check against.
    ...
    async #ReadOrFetchPayload(resolution, options)
    {
        if (!options.refresh)
        {
            ...
            // Validated when downloaded below and written atomically, so a
            // cached payload is trusted without re-hashing.
            if (cached)
            {
                return Object.freeze({ bytes: cached.bytes, cacheHit: true, cachePath: cached.cachePath });
            }
        }
