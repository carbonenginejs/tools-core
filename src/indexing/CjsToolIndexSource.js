...
export const CJS_INDEX_EXTERNAL_RESOLUTION = Symbol("CjsToolIndexExternalResolution");

/**
 * Cached, read-only remote payload source opened from one complete index graph.
 */
export class CjsToolIndexSource
{
    ...
    {
        if (!(indexes instanceof CjsToolIndexGraph))
        {
            throw new TypeError("CjsToolIndexSource requires a CjsToolIndexGraph");
        }

        if (typeof fetch !== "function")
        {
            throw new TypeError("CjsToolIndexSource requires fetch");
        }

        if (cache !== null && !(cache instanceof CjsToolIndexCache))
        {
            throw new TypeError("CjsToolIndexSource cache must be a CjsToolIndexCache or null");
        }
