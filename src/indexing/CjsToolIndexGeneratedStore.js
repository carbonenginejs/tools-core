...
export class CjsToolIndexGeneratedStore
{
    ...
    constructor({ cache } = {})
    {
        if (!(cache instanceof CjsToolIndexCache))
        {
            throw new TypeError(
                "CjsToolIndexGeneratedStore cache must be a CjsToolIndexCache",
            );
        }
    ...
    async OpenTarget(targetValue, buildValue, expected = {})
    {
        ...
        for (const descriptor of Object.values(manifest.groups))
        {
            ...
            groups.push(new CjsToolIndexOverlay({
                schema: ManifestSchema,
                version: ManifestVersion,
                target,
                game,
...
function NormalizeEntries(value)
{
    ...
    const paths = new Set();
    const entries = value.map(item => CjsToolIndexEntry.from(item));
